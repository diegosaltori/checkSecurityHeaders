...
def test_endpoint_cookies(logger, url, cookies, num_requests, delay, payload=None):
    """
    Sends multiple requests to the specified API endpoint using cookies with a delay and checks rate limit headers.
    Logs the results using the provided logger.

    Args:
        logger (logging.Logger): The logger object.
        url (str): The API endpoint URL.
        cookies (dict): A dictionary of cookies for authentication.
        num_requests (int): The number of requests to send.
        delay (float): The delay in seconds between each request.
        payload (dict, optional): The request body as a dictionary (for POST, PUT, etc.). Defaults to None.
    """

    logger.info(f"--- Starting Sequential Test with Cookies for {url} ---")
    success_count = 0
    error_count = 0
    ...
    logger.info(f"--- Sequential Test Results with Cookies for {url} ---")
...
def test_endpoint_concurrent_cookies(logger, url, cookies, num_requests, num_threads, delay_between_threads, payload=None):
    """
    Sends multiple concurrent requests to the specified API endpoint using cookies and checks rate limit headers.
    Logs the results using the provided logger.
    ...
    threads = []
    results = []
    logger.info(f"--- Starting Concurrent Test with Cookies for {url} with {num_threads} threads ---")
        ...
        for i in range(requests_to_send):
            try:
                if payload:
                    ...
                    response = requests.get(url, cookies=cookies)
                log_message = f"Thread {thread_id}, Request {i+1}: Status Code: {response.status_code}"
    ...
    logger.info(f"--- Concurrent Test Results with Cookies for {url} ---")
...
if __name__ == "__main__":
    ...
    bearer_token = "YOUR_BEARER_TOKEN"  # If auth_type is 'bearer'
    cookies_dict = {
        "cookies": "Put your cookies"
    }

    url = "API_URL" # uses cookies
    ...
    # --- Sequential Test with Cookies ---
    print("--- Starting Sequential Test with Cookies ---")
    logger.info("--- Starting Sequential Test with Cookies ---")
    num_sequential_requests_cookies = 10
    delay_between_requests_cookies = 0.5  # Time in seconds between requests
    test_endpoint_cookies(logger, url, cookies_dict, num_sequential_requests_cookies, delay_between_requests_cookies, payload=payload)

    print("\n" * 2)
    logger.info("\n" * 2)

    # --- Concurrent Test with Cookies ---
    print("--- Starting Concurrent Test with Cookies ---")
    logger.info("--- Starting Concurrent Test with Cookies ---")
    total_concurrent_requests_cookies = 20
    num_concurrent_threads_cookies = 5
    delay_between_thread_start_cookies = 0.2 # Time in seconds between the start of each thread
    test_endpoint_concurrent_cookies(logger, url, cookies_dict, total_concurrent_requests_cookies, num_concurrent_threads_cookies, delay_between_thread_start_cookies, payload=payload)